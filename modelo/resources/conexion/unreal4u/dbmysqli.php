...
include_once(dirname(__FILE__).'/auxiliar_classes.php');
include_once(dirname(__FILE__).'/exceptions/database.php');
include_once(dirname(__FILE__).'/exceptions/query.php');
        ...
        if (!$this->_error) {
            ...
            if (is_object($result_metadata)) {
                ...
                foreach ($result as $registro) {
                    array_push($tabla, $registro);
                }
                
                //$fields->close();
                //$_SESSION["mensaje2"]=$this->_stmt;
                $this->_stmt->close();
                ...
                unset($fields);
                unset($result);
                //$_SESSION["mensaje"]=$this->_stmt;

            } elseif ($this->_stmt->errno == 0) {
                $tabla = true;
            } else {
                $tabla = $this->_stmt->errno;
            }
        }
